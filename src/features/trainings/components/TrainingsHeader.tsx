...
export const TrainingsHeader: React.FC<TrainingsHeaderProps> = ({
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      {/* Premium Hero Banner */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-gradient-to-r from-emerald-950 via-slate-900 to-emerald-950 p-6 md:p-8 rounded-3xl text-white shadow-xl relative overflow-hidden border border-emerald-800/40">
        ...
        <div className="relative z-10 space-y-2">
          ...
          {nextSession && (
            <div className="pt-2 flex flex-wrap items-center gap-3 text-xs text-slate-300">
              <span className="bg-emerald-500/20 text-emerald-300 font-semibold px-3 py-1 rounded-full border border-emerald-500/30 flex items-center gap-1.5">
                <Clock className="w-3.5 h-3.5" />
                Prochaine séance : {nextSession.name} ({nextSession.team})
              </span>
              <span className="flex items-center gap-1 text-slate-400">
                <Calendar className="w-3.5 h-3.5" /> {nextSession.session_date} à {nextSession.start_time}
              </span>
              <span className="flex items-center gap-1 text-slate-400">
                <MapPin className="w-3.5 h-3.5" /> {nextSession.pitch}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="relative z-10 flex flex-wrap items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-800/80 hover:bg-slate-700 text-slate-200 border border-slate-700 hover:border-slate-600 font-semibold text-sm transition-all duration-200 shadow-sm hover:shadow active:scale-95"
          >
